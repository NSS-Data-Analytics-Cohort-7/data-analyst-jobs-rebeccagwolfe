...
--(Key board short cut: fn + F5 to execute query on laptop keyboard)
...
--"Microsoft"
--"Kaiser Permanente"
--All have 4.20 rating


/*
Question 11: Find all the job titles that contain the word ‘Analyst’. How many different job titles are there? 774
*/
...
SELECT title, COUNT(DISTINCT title)
FROM data_analyst_jobs
WHERE title LIKE '%Analyst%' OR LOWER (title) LIKE '%analyst%'
GROUP BY title
--774
...
SELECT title, COUNT(DISTINCT title)
FROM data_analyst_jobs
WHERE title LIKE '%Analyst%' OR title LIKE '%analyst%' OR title like '%ANALYST%'
GROUP BY title
--774
...
--Bonus: Internet and Software, Banks and Financial Services, Consulting and Business Services, and Health Care (62,61,57,52)
